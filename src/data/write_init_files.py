...
def write_phys_read_subroutine(outfile, host_dict, host_vars, host_imports,
    ...
    outfile.comment("Don't read the variable in if it's already initialized. This must", 3)
    outfile.comment("cover both read paths below: an advected constituent read by the", 3)
    outfile.comment("dycore is marked initialized but need not be a registry variable,", 3)
    outfile.comment("and reading it here would use the physics grid rather than the", 3)
    outfile.comment("dynamics grid it was written on.", 3)
    outfile.write("if (is_initialized(std_name)) then", 3)
