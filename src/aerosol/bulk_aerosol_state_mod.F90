...
!--------------------------------------------------------------------------------
! Updated for CAM-SIMA
! based on original abstract aerosol implementation in CAM from Francis Vitt.
! 1) rad_constituents has been replaced with the CAM-SIMA version.
! 2) physics buffer and physics state (CAM-specific) were removed since rad_constituents
!    uses the CAM-SIMA/CCPP constituents object infrastructure instead.
!
module bulk_aerosol_state_mod
  ...
  type, extends(aerosol_state) :: bulk_aerosol_state
    ...
    ! previously for rad_constituents::rad_cnst_get_aer_mmr
