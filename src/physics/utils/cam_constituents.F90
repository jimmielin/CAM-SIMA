module cam_constituents
   ...
   ! Tracks constituents whose initial values have already been provided, e.g.
   ! read by the dycore on the dynamics grid, so that the initial conditions
   ! read does not overwrite them on the physics grid. phys_vars_init_check
   ! cannot track these: its arrays cover registry variables only, while
   ! constituents may be registered at run time.
...
CONTAINS
   ...
   subroutine const_mark_as_initialized(const_ind)
      ...
      ! set, so that the initial conditions read leaves it alone. Used by
      ! dycores, which read the advected constituents on the dynamics grid.
